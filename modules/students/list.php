<?php
// Dòng 1: ĐẢM BẢO KẾT NỐI DB ĐƯỢC LOAD ĐÚNG CÁCH
...
<style>
    ...
    table th:nth-child(7), table td:nth-child(7), /* Lớp */
    ...
    table img {
        ...
        display: block; /* Quan trọng để căn giữa và quản lý lề */
...
<table>
    ...
    <?php while($row = $res->fetch_assoc()): 
        ...
        $photo_db = $row['photo'] ?? ''; // Giá trị DB: public/uploads/ten_file.png
        
        // LOGIC ẢNH: Đã sửa để dùng đường dẫn tương đối Web
        if ($photo_db) {
    ...
    <tr>
        ...
        <td><?= esc($row['email'] ?? '') ?></td>
        <td><?= esc($row['phone'] ?? '') ?></td>
        <td><?= esc($row['class_name'] ?? 'N/A') ?></td>
        <td>
            <a href="?url=student/edit&id=<?= $row['id'] ?>">Sửa</a>
            <a href="?url=student/delete&id=<?= $row['id'] ?>" onclick="return confirm('Bạn chắc chắn muốn xóa sinh viên này?')">Xóa</a>
        </td>
    </tr>
    <?php endwhile; ?>
    </tbody>
