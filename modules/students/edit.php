...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name) {
        $u = $conn->prepare("UPDATE students SET name=?,email=?,phone=?,class=?,photo=? WHERE id=?");
        $u->bind_param('sssssi',$name,$email,$phone,$class,$photo,$id);
        if ($u->execute()) {
...
<form method="post" enctype="multipart/form-data">
    ...
    <div class="form-row"><label>Lớp</label><input name="class" value="<?= esc($row['class']) ?>"></div>
